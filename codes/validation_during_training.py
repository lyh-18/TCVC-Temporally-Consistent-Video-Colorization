...
def validation_during_training(keynet, experiment_path, checkpoint):
    #################
    # configurations
    #################
    ...
    if data_mode == "DAVIS30":
        ...
        if key_net == "sig17":
            N_in = 1
            model_path = (
                "{}/{}_G.pth".format(experiment_path['models'], checkpoint)
            )
        ...
        if key_net == "unet_half_one_dm":
            N_in = 1
            model_path = (
                "../experiments/002_BRIB_UNet_half_dm_flownet2_Interframe_3_Videvo_2e-5/models/19000_G.pth"
            )
            model = BRIB_arch.BRIB(nf=64, N_RBs=2, key_net="unet_half_one_dm", dataset="videvo2")
    else:
    ...
    save_folder = "{}/{}_G".format(experiment_path['val_images'], checkpoint)
...
#             out_ab_rs = F.interpolate(out_ab, size=[H, W], mode="bilinear")
            ...
            save_imglist(k, k+len(out_rgb_img), save_subfolder, out_rgb_img, logger)
    ...
    for (
        ...
        logger.info("Folder {} - Average PSNR: {:.6f} dB.".format(video_list, psnr))
    logger.info("################ Final Results ################")
    logger.info("Data: {} - {}".format(data_mode, GT_dataset_folder))
    logger.info("Padding mode: {}".format(padding))
    logger.info("Model path: {}".format(model_path))
    ...
    with open('{}/val_log.txt'.format(experiment_path["experiments_root"]), 'a') as f:
        ...
        f.write('\n')
    return sum(avg_psnr_l) / len(avg_psnr_l), np.mean(JS_b_mean_list), np.mean(JS_g_mean_list), np.mean(JS_r_mean_list), CDC

if __name__ == '__main__':
    experiment_path = {}
    experiment_path['val_images'] = '../test'
    experiment_path['models'] = '../experiments/F01_BRIB_Sig17_fixColor_Videvo_v9_trainIn10_testIn11_warploss2_noCR/models'
    experiment_path["experiments_root"] = '../test'
    print(experiment_path)
    validation_during_training(keynet='sig17', experiment_path=experiment_path, checkpoint='8000')
