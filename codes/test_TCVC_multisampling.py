...
def save_imglist(k, end_k, output_dir, img_list, logger):
...
def append_imglist(k, end_k, img_list, all_result_imgs_list):
    """The color type of input img list is rgb"""
    count = 0
    for i in range(k, end_k):
        all_result_imgs_list[i] = img_list[count]
        count += 1
        
    return all_result_imgs_list
...
def main():
    ...
    save_folder = "../results/TCVC_{}_{}_ensemble{}_{}".format(key_net, data_mode, interval_length, interval_length2)
    ...
    for i in range(len(video_list)):
        ...
        all_result_imgs_list1 = [0]*len(imgs)
        all_result_imgs_list2 = [0]*len(imgs)
        
        def smpling_anchorframes_one(interval_length, all_result_imgs_list):        
    ...
    #             out_ab_rs = F.interpolate(out_ab, size=[H, W], mode="bilinear")
                ...
                out_rgb_img_uint8 = [util.tensor2img(np.clip(out_rgb_origsize[i,...]*255., 0, 255), np.uint8) for i in range(out_rgb_origsize.size(0))] # (0, 255)
                out_rgb_img = [util.tensor2img(out_rgb_origsize[i,...]*255., np.float64) for i in range(out_rgb_origsize.size(0))] # (0, 255)
                ...
                append_imglist(k, k+len(out_rgb_img), out_rgb_img, all_result_imgs_list)
            return all_result_imgs_list
        
        all_result_imgs_list1 = smpling_anchorframes_one(interval_length, all_result_imgs_list1)
        all_result_imgs_list2 = smpling_anchorframes_one(interval_length2, all_result_imgs_list2)
        
        save_img_list = []
        for i in range(len(all_result_imgs_list1)):
            img_1 = all_result_imgs_list1[i]
            img_2 = all_result_imgs_list2[i]
            avg_img = (img_1+img_2)/2.0
            avg_img = np.clip(avg_img, 0, 255).astype(np.uint8)
            save_img_list.append(avg_img)
        
        save_imglist(0, len(imgs), save_subfolder, save_img_list, logger)
