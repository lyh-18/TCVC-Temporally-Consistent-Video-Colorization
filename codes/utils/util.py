...
def flipxrot_forward(model, inp):
    # normal
    output_f = single_forward(model, inp)
    ...
    # rot90
    output = single_forward(model, torch.rot90(inp, 1, (-1, -2)))
    output_f = output_f + torch.rot90(output, 3, (-1, -2))

    # rot270
    output = single_forward(model, torch.rot90(inp, 3, (-1, -2)))
    output_f = output_f + torch.rot90(output, 1, (-1, -2))

    # flip W rot90
    output = single_forward(model, torch.rot90(torch.flip(inp, (-1,)), 1, (-1, -2)))
    output_f = output_f + torch.flip(torch.rot90(output, 3, (-1, -2)), (-1,))

    # flip H rot90
    output = single_forward(model, torch.rot90(torch.flip(inp, (-2,)), 1, (-1, -2)))
    output_f = output_f + torch.flip(torch.rot90(output, 3, (-1, -2)), (-2,))

    # flip both H and W rot90
    output = single_forward(model, torch.rot90(torch.flip(inp, (-2, -1)), 1, (-1, -2)))
    output_f = output_f + torch.flip(torch.rot90(output, 3, (-1, -2)), (-2, -1))

    # flip W rot270
    output = single_forward(model, torch.rot90(torch.flip(inp, (-1,)), 3, (-1, -2)))
    output_f = output_f + torch.flip(torch.rot90(output, 1, (-1, -2)), (-1,))

    # flip H rot270
    output = single_forward(model, torch.rot90(torch.flip(inp, (-2,)), 3, (-1, -2)))
    output_f = output_f + torch.flip(torch.rot90(output, 1, (-1, -2)), (-2,))

    # flip both H and W rot270
    output = single_forward(model, torch.rot90(torch.flip(inp, (-2, -1)), 3, (-1, -2)))
    output_f = output_f + torch.flip(torch.rot90(output, 1, (-1, -2)), (-2, -1))
